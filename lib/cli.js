...
function setupGroup(group) {
    ...
    group.resourceSet.prependToLoad(["/buster-static-browser-wiring.js",
                                     "/buster-static-html-reporter.js"]);

    // Some neat CSS
    group.resourceSet.addFile(
        require.resolve("buster-test/resources/buster-test.css"),
...
function runWithConfigGroup(cli, group) {
    setupGroup(group);

    if (cli.path.isSet) {
        cli.writeToDisk(group);
    } else {
        cli.startServer(group);
    }
}
...
module.exports = buster.extend(bCli.create(), {
    ...
    startServer: function (group) {
        this.httpServer = startServer(group.resourceSet,
                                      this.port.value,
                                      this.logger);
    },
